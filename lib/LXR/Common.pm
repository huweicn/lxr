...
our @EXPORT = qw(
	...
	&warning &fatal
	&fflush
	&urlargs &nonvarargs &fileref &diffref &idref &idrefcall &idreflist &incref
	&httpinit &httpclean
);
# our @EXPORT_OK = qw(
#   &abortall 
...
#                $ENV{'QUERY_STRINGFID'}=sprintf(substr($value[2],2));
	#	$ENV{'QUERY_STRINGFID'}='vir' if substr($value[2],2) eq 'virtual';
		$ENV{'QUERY_STRINGFID'}='real' if substr($value[2],2) eq 'real';
...
#        $HTTP->{'param'}{'f'}   ||= $HTTP->{'param'}{'_virtu'}; ##add by twd 20131122
